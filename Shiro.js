...
module.exports = Shiro = async (Shiro, m, chatUpdate, store) => {
  try {
    ...
    const messageTypes = {
      ...
      extendedTextMessage:
        m.message?.extendedTextMessage?.text || "[Extended Text]",
    ...
    const sender = m.key.fromMe
      ? Shiro.user.id.split(":")[0] + "@s.whatsapp.net" || Shiro.user.id
    ...
    if (m.message) {
      ...
      console.log(
        chalk.bgHex("#0a0a0a").hex("#00ff00")(
          "┌───────────────── 🄼🄴🅂🅂🄰🄶🄴 ─────────────────┐",
        ),
      );
      console.log(
        chalk.bgHex("#0a0a0a").hex("#ff00ff")(
          `   ⚡ ${glitchText("INCOMING TRANSMISSION")}`,
        ),
      );
      console.log(
        chalk.bgHex("#0a0a0a").hex("#00ffff")(
          "├─────────────────────────────────────────────┤",
        ),
      );
    ...
    function countFiles(dir) {
      ...
      return fs
        .readdirSync(dir)
        .filter((f) => f.endsWith(".js") || f.endsWith(".mjs")).length;
    }
...
`;
      ...
      case "cekidch":
        {
          if (!text)
            return Shiro.sendMessage(
              m.chat,
              {
                text: "Masukkan link channel.\nContoh:\n.cekidch https://whatsapp.com/channel/xxxx",
              },
              { quoted: m },
            );

          let code = text.split("/channel/")[1];
          ...
          try {
            let res = await Shiro.newsletterMetadata("invite", code);
         ...
         `;
            ...
            Shiro.sendMessage(m.chat, { text: teks }, { quoted: m });
          } catch (e) {
            Shiro.sendMessage(
              m.chat,
              { text: "Channel tidak ditemukan atau private." },
              { quoted: m },
            );
          }
        }
...
`;
                  ...
                  interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    header: proto.Message.InteractiveMessage.Header.create({
                      ...
                      ...(await prepareWAMessageMedia(
                        {
                          image: {
                            url: "https://i.ibb.co.com/ZRLdWCnJ/IMG-20260316-WA0023.jpg",
                          },
                        },
          ...
          await Shiro.relayMessage(msg.key.remoteJid, msg.message, {
      ...
      case "setppbot":
        {
          ...
          if (!/image/.test(mime))
            return reply(`Kirim/kutip gambar dengan caption ${cmd}`);
          if (/webp/.test(mime))
            return reply(`Kirim/kutip gambar dengan caption ${cmd}`);
          let media = await sock.downloadAndSaveMediaMessage(quoted);
          await sock
            .updateProfilePicture(botNumber, {
              url: media,
            })
            .then(() => fs.unlinkSync(media))
      ...
      case "addown":
        {
          ...
          let raw = m.quoted ? m.quoted.sender.split("@")[0] : text;
          ...
          if (owner.includes(jid))
            return reply(`Nomor ${number} sudah menjadi owner!`);
          if (jid === botNumber)
            return reply(`Tidak bisa menambahkan bot sebagai owner!`);
          owner.push(jid);
      ...
      case "remini":
        {
          ...
          if (!/image/.test(mime))
            return reply(`Send/Reply Foto dengan caption ${cmd}`);
      ...
      case "cjs2esm":
        {
          ...
          if (!text)
            return reply(
              "Kirim kode CJS atau reply file JS!\n\nContoh:\n.cjs2esm const fs = require('fs')",
            );
      ...
      case "promote":
        {
          ...
          if (m.quoted || text) {
            ...
            await sock
              .groupParticipantsUpdate(m.chat, [target], action.toLowerCase())
              .then(async () => {
                await sock.sendMessage(
                  m.chat,
                  {
        ...
        try {
          async function fetchInitialPage(initialUrl) {
            try {
              ...
              const headers = {
                "User-Agent":
                  "Mozilla/5.0 (Linux; Android 10; RMX2185 Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.7103.60 Mobile Safari/537.36",
                Referer: initialUrl,
              };
          ...
          async function postDownloadRequest(
            ...
            try {
              ...
              if (response.data?.result?.length) {
                ...
                throw new Error(
                  "Respons dari server gak sesuai harapan, coba link lain!",
                );
              }
            } catch (error) {
              throw new Error(
                `Gagal proses permintaan download: ${error.message}`,
              );
            }
          }
          async function sendMediaAutoType(url, title) {
            try {
              ...
              if (mime.startsWith("video/")) {
                await sock.sendMessage(
                  m.chat,
                  { video: buff, caption: title },
                  { quoted: m },
                );
              } else if (mime.startsWith("audio/")) {
                await sock.sendMessage(
                  m.chat,
                  { audio: buff, mimetype: mime },
                  { quoted: m },
                );
              } else if (mime.startsWith("image/")) {
                await sock.sendMessage(
                  m.chat,
                  { image: buff, caption: title },
                  { quoted: m },
                );
              } else {
                await sock.sendMessage(
          ...
          const results = await postDownloadRequest(
            downloadUrl,
            q,
            csrfToken,
            cookies,
...
fs.watchFile(currentFile, () => {
  ...
  console.log(
    chalk.green(`✓ ${path.basename(currentFile)} updated! Reloading...`),
  );
