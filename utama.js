...
async function connectToWhatsApp() {
  ...
  if (!Shiro.authState.creds.registered) {
    ...
    const code = await Shiro.requestPairingCode(phoneNumber, "SHIROOMD");
  ...
  Shiro.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (!Shiro.public && !mek.key.fromMe && chatUpdate.type === "notify")
        return;
  ...
  Shiro.sendText = (jid, text, quoted = "", options) =>
    Shiro.sendMessage(jid, { text, ...options }, { quoted });
  ...
  Shiro.downloadAndSaveMediaMessage = async (
    message,
    filename,
...
END:VCARD`,
  ...
  Shiro.sendMedia = async (
    ...
    let { mime, data } = await Shiro.getFile(path, true);
    ...
    await Shiro.sendMessage(jid, messageContent, { quoted });
  };

  Shiro.sendPoll = async (jid, question, options) => {
    const pollMessage = {
      pollCreationMessage: {
        name: question,
    ...
    await Shiro.sendMessage(jid, pollMessage);
  };

  Shiro.public = true;

  Shiro.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      Shiro.newsletterFollow("120363402508663272@newsletter");
      Shiro.newsletterFollow("120363423625415506@newsletter");
      Shiro.newsletterFollow("120363424163797384@newsletter");
    }
  });

  Shiro.ev.on("error", (err) => {
    console.error(chalk.red("Error: "), err.message || err);
  });

  Shiro.ev.on("creds.update", saveCreds);
}
connectToWhatsApp();
